...
// 搜索栏js
// 切换搜索引擎表单的函数
function openCustomTab(event, searchEngine) {
  // 隐藏所有搜索表单
  var forms = document.getElementsByClassName('custom-search-form');
  for (var i = 0; i < forms.length; i++) {
    forms[i].classList.remove('active');
  }

  // 移除所有按钮的 active 类
  var buttons = document.getElementsByClassName('custom-tab-button');
...
// 提交前检查输入框内容
function handleSubmit(inputName, homepageUrl) {
  var searchInput = document.querySelector(`input[name="${inputName}"]`);
